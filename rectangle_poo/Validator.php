...
//Expressions Régulières
    public function  is_email($valeur,$key,$sms=null){
        if (!preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#",  $valeur)) {
            if ($sms == null) {
                $sms = "Addresse E-mail incorrecte !";
            }
            $this->errors[$key] = $sms;
        }
    }
    ...
    public function  is_telephone($valeur,$key,$sms=null){
        if (!preg_match('#^7[7-8560][ -]?[0-9]{3}([ -]?[0-9]{2}){2}$#', $valeur)) {
            if ($sms == null) {
                $sms = "Mauvais Numéro de téléphone ";
            }
            $this->errors[$key] = $sms;
        }
    }
